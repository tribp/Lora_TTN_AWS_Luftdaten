...
void loop() {
  digitalWrite(13, HIGH);
    ...
    byte payload[10];
    //Devicetype 
    payload[0] = highByte(DEVICETYPE);
    payload[1] = lowByte(DEVICETYPE);
    // sds011
    payload[2] = highByte(p10int);
    payload[3] = lowByte(p10int);
    payload[4] = highByte(p25int);
    payload[5] = lowByte(p25int);
    // dht22
    payload[6] = highByte(hint); // humidity
    payload[7] = lowByte(hint);
    payload[8] = highByte(tint); // temperature
    payload[9] = lowByte(tint);
